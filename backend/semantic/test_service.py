...
def test_error_cases():
    ...
    response = requests.post(f"{BASE_URL}/search", json={"query": "", "k": 5})
    print(f"   Status: {response.status_code} (expected 400)")
    
    # Invalid k
    print("\n2. Testing invalid k...")
    response = requests.post(f"{BASE_URL}/search", json={"query": "test", "k": -1})
    print(f"   Status: {response.status_code} (expected 400)")
    
    # Missing file_id
    print("\n3. Testing missing file_id...")
    response = requests.post(f"{BASE_URL}/index", json={"file_id": "", "text": "test"})
    print(f"   Status: {response.status_code} (expected 400)")
    
    # Empty text
    print("\n4. Testing empty text...")
    response = requests.post(f"{BASE_URL}/index", json={"file_id": "test123", "text": ""})
    print(f"   Status: {response.status_code} (expected 400)")
